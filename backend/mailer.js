...
async function sendEnquiryEmail(enquiryData) {
  ...
  if (!fromEmail || !toEmail) {
    console.warn('AWS SES: SES_FROM_EMAIL or SES_TO_EMAIL is not set. Skipping AWS SES transmission.');
    console.log('========================================================================');
    ...
    console.log(`Subject: New Quotation Enquiry - ${enquiryData.product_interest} from ${enquiryData.contact_name}`);
    console.log('------------------------------------------------------------------------');
  ...
  try {
    ...
    console.error('AWS SES: Failed to send emails via SES. Error:', error.message);
